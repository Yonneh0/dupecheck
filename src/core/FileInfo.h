...
// Unix epoch offset in seconds (Jan 1, 1970 UTC) relative to FILETIME epoch.
constexpr long long EPOCH_OFFSET = 13477420800LL;

// Alias types used across the codebase.
...
namespace PathUtils {
    ...
    // Get long-form path with \\?\ prefix for paths >260 chars
    ...
    // Extract file extension (lowercased, without dot)
    ...
    // Get file mtime as FILETIME int64 (in seconds since epoch)
    ...
    // Enumerate files in directory recursively
    struct FileEntry {
        std::wstring path;
        uint64_t size;
        long long mtime;
    ...
    // Create a duplicates subfolder under the given directory.
