...
std::vector<FileInfo> CachedScannerService::scan(const wchar_t* path) {
    ...
    cache_.remove_deleted_files([&current_entries]() -> std::vector<std::wstring> {
        std::vector<std::wstring> paths;
        paths.reserve(current_entries.size());
        for (const auto& e : current_entries) paths.push_back(e.path);
        return paths;
    }());
